...
export default function CartItem({ item, onUpdateQuantity, onRemove }: CartItemProps) {
  ...
  return (
    <div className="flex items-center p-8 bg-white border-b border-gray-200 last:border-0 hover:bg-gray-50 transition-colors duration-300">
      <img 
      ...
      <div className="ml-8 flex-grow flex flex-col justify-between">
        <div className="flex justify-between">
          <div>
            <h3 className="text-xl font-serif italic text-gray-900 line-clamp-1">{product.name}</h3>
            <p className="text-gray-500 text-xs font-bold uppercase tracking-[0.2em] mt-2">{product.category}</p>
          </div>
          <p className="text-xl font-black font-serif text-gray-900">${product.price.toFixed(2)}</p>
        </div>
